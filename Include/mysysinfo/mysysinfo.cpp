...
bool MySysInfo::GetdiskSpace(unsigned long& lFreeAll, unsigned long& lTotalAll) {
	...
	while (!process.atEnd()) {
		...
		if (str.startsWith("/dev/sda")) {
			...
			auto lst = str.split(" ");
			if (lst.size() > 5)
				// qDebug("挂载点:%s 已用:%.0lfMB 可用:%.0lfMB", lst[5]helper::Qstr2str().c_str(), lst[2].toDouble() / 1024.0, lst[3].toDouble() / 1024.0);
	...
	for (int drive = 1; drive <= curdrive; drive++) //遍历所有盘符
	{
		if (!_chdrive(drive)) {
			...
			MultiByteToWideChar(CP_ACP, 0, path, static_cast<int>(strlen(path)) + 1, wszClassName, sizeof(wszClassName) / sizeof(wszClassName[0]));
