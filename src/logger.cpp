...
string fmtMotdBE(string msgJson, string returnMsg) {
	...
	if (msg["status"] == "online") {
		for (json::iterator it = msg.begin(); it != msg.end(); ++it) {
			...
			tempReturnMsg = helper::replace(tempReturnMsg, "{" + key + "}", value);
		}
...
string fmtMotdJE(string msgJson, string returnMsg) {
	...
	if (msg["status"] == "online") {
		...
		// Version
		string name = msg["g_VERSION"].get<string>();
		string protocal = std::to_string(msg["agreement"].get<int>());
		// delay
		string delay = std::to_string(msg["delay"].get<int>());
		tempReturnMsg = helper::replace(tempReturnMsg, "{motd}", motdjson);
		tempReturnMsg = helper::replace(tempReturnMsg, "{max}", max);
		tempReturnMsg = helper::replace(tempReturnMsg, "{online}", online);
		tempReturnMsg = helper::replace(tempReturnMsg, "{player}", player);
		tempReturnMsg = helper::replace(tempReturnMsg, "{g_VERSION}", name);
		tempReturnMsg = helper::replace(tempReturnMsg, "{agreement}", protocal);
		tempReturnMsg = helper::replace(tempReturnMsg, "{delay}", delay);
		tempReturnMsg = helper::replace(tempReturnMsg, "{host}", host);
		return tempReturnMsg;
	}
	else {
		return "发生错误!原因:服务器状态为" + msg["status"].get<string>();
...
QString fmtConsole::getColoredLine(QString qline) {
	bool getConsole = g_config["ConsoleColor"].as<bool>();
	try {
		if (getConsole) {
			qline = qline.replace("<", "&lt;");
			qline = qline.replace(">", "&gt;");
			...
			//删除多余的</font>
			string lline = "</font>";
			auto start = qline.mid(0, static_cast<int>(lline.length()));
			if (start == "</font>") {
